...
class MarketPlaceSellerAccountSerializer(serializers.ModelSerializer):
    ...
    seller_joined_since = serializers.CharField(
        source='seller.created_at', read_only=True)
    user_last_login = serializers.CharField(
        source='seller.last_login', read_only=True)
...
class MessageSerializer(serializers.ModelSerializer):
    ...
    buyer_first_name = serializers.CharField(
        source='buyer.first_name', read_only=True)
    buyer_username = serializers.CharField(
        source='buyer.username', read_only=True)
    seller_first_name = serializers.CharField(
        source='seller.first_name', read_only=True)
    seller_username = serializers.CharField(
        source='seller.username', read_only=True)
    free_ad_id = serializers.CharField(source='free_ad.id', read_only=True)
    free_ad_name = serializers.CharField(
        source='free_ad.ad_name', read_only=True)
    free_ad_price = serializers.CharField(
        source='free_ad.price', read_only=True)
    free_ad_currency = serializers.CharField(
        source='free_ad.currency', read_only=True)
    free_ad_expiration_date = serializers.CharField(
        source='free_ad.expiration_date', read_only=True)
    free_ad_rating = serializers.CharField(
        source='free_ad.ad_rating', read_only=True)
    free_ad_image1 = serializers.CharField(
        source='free_ad.image1.photo.url', read_only=True)
    free_seller_username = serializers.CharField(
        source='free_ad.seller.username', read_only=True)
    free_ad_message_id = serializers.CharField(
        source='free_ad_message_id.free_ad_message_id', read_only=True)

    sellerAvatarUrl = serializers.CharField(
        source='free_ad.seller.photo.url', read_only=True)

    paid_ad_id = serializers.CharField(source='paid_ad.id', read_only=True)
    paid_ad_name = serializers.CharField(
        source='paid_ad.ad_name', read_only=True)
    paid_ad_id = serializers.CharField(source='paid_ad.id', read_only=True)
    paid_ad_name = serializers.CharField(
        source='paid_ad.ad_name', read_only=True)
    paid_ad_price = serializers.CharField(
        source='paid_ad.price', read_only=True)
    paid_ad_currency = serializers.CharField(
        source='paid_ad.currency', read_only=True)
    paid_ad_expiration_date = serializers.CharField(
        source='paid_ad.expiration_date', read_only=True)
    paid_ad_rating = serializers.CharField(
        source='paid_ad.ad_rating', read_only=True)
    paid_ad_image1 = serializers.CharField(
        source='paid_ad.image1.photo.url', read_only=True)
    paid_seller_username = serializers.CharField(
        source='paid_ad.seller.username', read_only=True)
    paid_ad_message_id = serializers.CharField(
        source='paid_ad_message_id.paid_ad_message_id', read_only=True)
...
class FreeAdMessageIdSerializer(serializers.ModelSerializer):
    ...
    free_ad_rating = serializers.CharField(
        source='free_ad.ad_rating', read_only=True)
    free_ad_seller_username = serializers.CharField(
        source='free_ad.seller.username', read_only=True)
...
class PaidAdMessageIdSerializer(serializers.ModelSerializer):
    ...
    paid_ad_rating = serializers.CharField(
        source='paid_ad.ad_rating', read_only=True)
    paid_ad_seller_username = serializers.CharField(
        source='paid_ad.seller.username', read_only=True)
